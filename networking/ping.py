# ping.py
# ref: https://github.com/moranzcw/Computer-Networking-A-Top-Down-Approach-NOTES
import socket
import os
import struct
import time
...
def receiveOnePing(mySocket, ID, sequence, destAddr, timeout):
    ...
    while 1:
        ...
        # TODO: select: https://docs.python.org/3/library/select.html
        ...
        # Fill in start
        header = recPacket[20: 28] # size of IPV4 header is 20 bytes, and ping header is 8 bytes;
        # struck.unpack() : https://docs.python.org/3/library/struct.html#format-characters
        # ICMP echo reply/request format:
...
def sendOnePing(mySocket, ID, sequence, destAddr):
    # Header is type (8), code (8), checksum (16), id (16), sequence (16), format:bbHHh
...
def doOnePing(destAddr, ID, sequence, timeout):
    ...
    # Fill in start
    mySocket = socket.socket(socket.AF_INET, socket.SOCK_RAW, icmp) # create raw socket
    # Fill in end
